...
inactiveFirstLocation.addEventListener('click', () => {
    ...
    switch (locationOption) {
        case 'St.Gallen':
            ...
            inactiveSecondLocation.innerHTML = 'Zurich';
        ...
        case 'All Locations':
            ...
            inactiveFirstLocation.innerHTML = 'St.Gallen';
            inactiveSecondLocation.innerHTML = 'Zurich';
            filteringLocation(locationOption);
            //50ms delay for closing animation
            setTimeout(() => closeDropdownLocation(),50); 
...
inactiveSecondLocation.addEventListener('click', () => {
    ...
    switch(locationOption) {
        case 'St.Gallen':
            ...
            break;
        case 'Zurich':
            activeLocation.innerHTML = 'Zurich <i class="bi bi-caret-up-fill icon_arrow_loc"></i>';
            inactiveFirstLocation.innerHTML = 'All Locations';
...
inactiveCurrency.addEventListener('click', () => {
    ...
    if (inactiveCurrency.innerHTML === 'USD') {
        activeCurrency.innerHTML = 'USD <i class="bi bi-caret-up-fill icon_arrow_curr active"></i>';
    ...
    // Else switch active currency to CHF
        ...
        inactiveCurrency.innerHTML = 'USD';
        currency = 'CHF';
    } 
    //50ms delay for closing animation
        ...
        switch (currency) {
            case 'USD':
                ...
                .then(convertedPrice => {
                    ...
                    displayPrice.innerHTML = 'Fee: ' + `${roundedPrice}` + ' USD per hour';
                    displayPrice.dataset.usd = `${roundedPrice}`;
                });
                break;
            case 'CHF':
...
async function getExchangeRate(price) {
    ...
    // Fetch exchange rate 
    try {
        const response = await fetch(`https://shareadesk-api.onrender.com/exchangerate/${price}/${convertCurrencyFrom}/${convertCurrencyTo}`)
        const data = await response.json();
        const convertedPrice = data.data.conversion_result;
...
async function validateAvailability(datasetId, valueDate, valueStartTime, valueEndTime) {
    ...
    const response = await fetch(`https://shareadesk-api.onrender.com/bookings?deskid=${datasetId}&start=${start}&end=${end}&studid=${studentId}`,requestOptions);
    const data = await response.json();
...
async function getAllDesks() {
    ...
    try {
        const response = await fetch('https://shareadesk-api.onrender.com/desks', requestOptions);
        const data = await response.json();
        return data;
    } catch (error) {
...
async function postBooking(formData) {
    ...
    //FETCH
    try {
        const response = await fetch('https://shareadesk-api.onrender.com/booking', requestOptions);
        const data = await response.json();
...
async function deleteBooking(bookingReferenceId) {
    ...
    //FETCH
    try {
        const response = await fetch(`https://shareadesk-api.onrender.com/booking/${bookingReferenceId}`, requestOptions);
        const data = await response.json();
    } catch (error) {
        console.log(error);
    }
...
async function getAllReservations(deskId) {
    ...
    //FETCH
    try {
        const response = await fetch('https://shareadesk-api.onrender.com/bookings?deskid=' + `${deskId}` + '&start=' + `${start}` + '&end=' + `${end}` + '&studid=' + `${studentId}`, requestOptions);
        const data = await response.json();
        //Return data
        return data;
    } catch (error) {
...
function renderReservations(reservationsSortedAfterDate) {
    ...
    for (let i = 0; i < reservationsSortedAfterDate.length; i++) {
        ...
        //Creating map button
        const buttonMap = document.createElement('button');
        buttonMap.id = 'btn_res_map_id' + `${bookingReferenceId}`;
        buttonMap.dataset.button = 'map';
